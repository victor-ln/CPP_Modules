...
inline bool    Sed::_createFileToWrite(void) {
    ...
    this->_fileToWrite.open(
        this->_replaceFileName.c_str(),
        std::ios::out | std::ios::trunc);
    return (this->_fileToWrite.is_open());
}

inline void    Sed::_replace(std::size_t pos, std::string* buffer) {
    *buffer = buffer->substr(0, pos) + this->_replacement + \
            buffer->substr(pos + this->_strToFind.length());
}
...
std::string    Sed::_readFromFile(void) {
    ...
    while (std::getline(this->_fileToRead, readBuffer)) {
        lastPos = 0;
        while ((pos = readBuffer.find(this->_strToFind)) != std::string::npos \
                && pos >= lastPos) {
